...
class MinIOService:
    ...
    def generate_download_url(self, s3_key: str, expires: int = 3600, inline: bool = False) -> Optional[str]:
        """生成文件下载URL（预签名URL）
        
        Args:
            s3_key: S3对象键
            expires: URL过期时间（秒），默认1小时
            inline: 是否强制内联显示（用于PDF等文档）
        ...
        try:
            ...
            # 生成预签名URL - use timedelta for newer MinIO versions
            ...
            logger.info(f"Generated {'inline' if inline else 'download'} URL for {s3_key}, expires in {expires} seconds")
            return url
            ...
            logger.error(f"Unexpected error generating download URL for {s3_key}: {e}")
            return None
