        ...
        try:
            ...
            for file_metadata in files:
                try:
                    ...
                    # 5. 如果失败，回滚该文件的操作
                    db.rollback()
                    logger.error(f"Failed to process file {file_metadata.id} ({file_metadata.original_filename}): {e}")
                    
                    # 创建FileExtractionFailure记录
                    failure_record = FileExtractionFailure(
                        task_id=task_id,
                        filename=file_metadata.original_filename,
                        error=f"Processing job creation failed: {str(e)}",
            ...
            if failed_files:
                logger.warning(f"Failed to create {len(failed_files)} jobs for task {task_id}")
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                with tqdm(total=total_files, desc=f"Task {task_id} [EXTRACTING]", unit="files", 
                    ...
                    for file_info in zip_ref.infolist():
                        ...
                        try:
                            # 记录当前处理的文件信息
                            logger.info(f"Processing file: {file_info.filename} (size: {file_info.file_size} bytes)")
