...
from app.api import tasks, extractions
...
async def lifespan(app: FastAPI):
    ...
    # 启动Kafka消费者服务（强依赖）
    try:
        await kafka_consumer_service.start_async_consumer()
    ...
    # 停止字段提取消费者服务
    try:
        await field_extraction_consumer.stop_async_consumer()
        logger.info("Field extraction consumer service stopped")
    except Exception as e:
        logger.error(f"Error stopping field extraction consumer service: {e}")
    
    # 停止Kafka消费者服务
    try:
        await kafka_consumer_service.stop_async_consumer()
